...
function determinePriority(failure) {
  const fileKey = (failure.file || '').toLowerCase();
  const title = (failure.title || '').toLowerCase();

  if (fileKey.includes('login') || title.includes('login') || title.includes('authentication') || title.includes('timeout')) {
    return 'High';
  }
  if (fileKey.includes('visual') || title.includes('visual')) {
    return 'Low';
  }
  return 'Medium';
}
...
function getDateLabel() {
  return `run-date-${new Date().toISOString().slice(0, 10)}`;
}
...
async function createJiraTickets(failures) {
  ...
  for (const failure of failures) {
    ...
    const labels = ['automation', 'regression', 'stlc-pipeline', dateLabel];
    ...
    try {
      ...
      console.log(`  Created: ${ticket.key} - ${summary} (${priority})`);
    } catch (error) {
