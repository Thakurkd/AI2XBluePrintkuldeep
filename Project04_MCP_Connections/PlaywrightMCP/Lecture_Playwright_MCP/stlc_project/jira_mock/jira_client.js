...
class JiraClient {
  ...
  async isHealthy() {
    try {
      let response = await fetch(`${this.baseURL}/health`, { headers: this.headers });
      if (response.ok) {
        return true;
      }

      // Jira Cloud may not expose /health, so try an authenticated endpoint.
      response = await fetch(`${this.baseURL}/rest/api/2/myself`, { headers: this.headers });
      return response.ok;
    } catch {
      return false;
    }
